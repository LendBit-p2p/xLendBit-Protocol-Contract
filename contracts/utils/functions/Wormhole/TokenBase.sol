...
abstract contract TokenBase is AppStorage {}
...
abstract contract TokenSender is TokenBase {
    ...
    function sendTokenWithPayloadToEvm(
        ...
        (uint256 cost, ) = LibXGetters
            ._wormholeRelayer(_appStorage)
            .quoteEVMDeliveryPrice(targetChain, receiverValue, gasLimit);
        return
            LibXGetters._wormholeRelayer(_appStorage).sendVaasToEvm{
                value: cost
            }(
                targetChain,
                targetAddress,
    ...
    function receiveWormholeMessages(
        ...
        for (uint256 i = 0; i < additionalVaas.length; ++i) {
            IWormhole.VM memory parsed = LibXGetters
                ...
                .parseVM(additionalVaas[i]);
            require(
                parsed.emitterAddress ==
                    LibXGetters._tokenBridge(_appStorage).bridgeContracts(
                        parsed.emitterChainId
                    ),
                "Not a Token Bridge VAA"
            );
            ITokenBridge.TransferWithPayload memory transfer = LibXGetters
                ._tokenBridge(_appStorage)
                .parseTransferWithPayload(parsed.payload);
            require(
                transfer.to == Utils.toUniversalAddress(address(this)) &&
                    transfer.toChain ==
                    LibXGetters._wormhole(_appStorage).chainId(),
                "Token was not sent to this address"
            );

            LibXGetters._tokenBridge(_appStorage).completeTransferWithPayload(
                additionalVaas[i]
            );
